                ...
                if context.scene.hisanimvars.savespace:
                    for NODE in mat.node_tree.nodes:
                        if NODE.type == 'TEX_IMAGE':
                            if NODE.image.library != None: continue
                            ReuseImage(NODE, PATH + f'/{self.merc}.blend')
                            continue
                        if NODE.type == 'GROUP' and NODE:
                            if NODE.node_tree.library != None: continue
                            if NODE.node_tree.name.isupper():
                                nodebak = NODE.node_tree.name
                                NODE.node_tree.name = NODE.node_tree.name.lower()
                            else:
                                nodebak = NODE.node_tree.name
                                NODE.node_tree.name = NODE.node_tree.name.upper()
                            
                            if (NEWNODE := bpy.data.node_groups.get(nodebak)) != None:
                                NODE.node_tree = NEWNODE
                                continue

                            link(os.path.join(PATH, f'scout.blend'), nodebak, 'NodeTree')
                            if (NEWNODE := bpy.data.node_groups.get(nodebak)) != None:
                                NODE.node_tree = NEWNODE
                                continue
                            else:
                                NODE.node_tree.name = nodebak
                                del nodebak
                                continue
